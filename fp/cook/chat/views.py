...
from chat.models import Chats, ChatSession  # ✅ Message → Chats 변경
from account.models import Users  # 사용자 정보
...
def chat_history(request, session_id):
    ...
    messages = Chats.objects.filter(session=chat_session).order_by("created_at")  # ✅ timestamp → created_at

    message_list = []
    for msg in messages:
        # 사용자 입력 메시지 추가
        message_list.append({
            "content": msg.question_content,  # ✅ content → question_content
            "sender": "User",
            "timestamp": msg.created_at.strftime("%Y-%m-%d %H:%M:%S")
        })
        # AI 응답이 있다면 추가
        if msg.response_content:  # ✅ response → response_content
            message_list.append({
    ...
    if request.method == "POST":
        try:
            ...
            Chats.objects.create(
                ...
                question_content=user_message,  # ✅ content → question_content
                response_content=response  # ✅ response → response_content
            )
    ...
    session_data = [
        {
            ...
            "latest_message": Chats.objects.filter(session=session).order_by("-created_at").first().question_content if Chats.objects.filter(session=session).exists() else "메시지 없음",
